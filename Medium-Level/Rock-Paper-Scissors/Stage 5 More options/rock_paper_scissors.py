...
class Game:
    ...
    def __init__(self):
        ...
        self.default_win_options = {"paper": ["rock"], "scissors": ["paper"], "rock": ["scissors"]}
        self.is_game = True
        ...
        if self.user_options:
            ...
            self.options = self.user_options.split(",")
            half = len(self.options) // 2
            doubled_options = self.options * 2
            self.win_options = \
                {key: doubled_options[-1 + len(self.options) + index: -1 - half + len(self.options) + index: -1]
                 for index, key in enumerate(self.options)}
        else:
            self.options = self.default_options
            self.win_options = self.default_win_options
        self.user_choice = ""
    ...
    def _is_user_win(self):
        return self.computer_choice in self.win_options[self.user_choice]

    def _is_computer_win(self):
        return self.user_choice in self.win_options[self.computer_choice]
