...
if sys.argv[1:4]:
    from_language = {value.lower(): key for key, value in supporting_languages.items()}.get(sys.argv[1], sys.argv[1])
    to_language = "0" if sys.argv[2] == "all" else\
        {value.lower(): key for key, value in supporting_languages.items()}.get(sys.argv[2], sys.argv[2])
...
if from_language not in supporting_languages:
    print(f"Sorry, the program doesn't support {from_language}")
elif to_language not in supporting_languages:
    print(f"Sorry, the program doesn't support {to_language}")
else:
    try:
        requests.get("https://context.reverso.net/")
        with open(f"{word_to_translate}.txt", "a+", encoding="utf-8") as file:
            if to_language == "0":
                for language in supporting_languages:
                    if language != from_language and language != "0":
                        result = translate(from_language, language, word_to_translate)
                        if result == 404:
